    ...
    <main class="flex-1 px-4 pt-24 sm:px-6 sm:pt-28 lg:pt-32">
      <div class="w-full max-w-7xl mx-auto">
        <div class="mb-8">
          ...
          <p class="mt-3 text-[#355347]">Comprehensive analytics and insights for program management.</p>
        </div>

        <!-- Performance Indicators -->
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          <div v-if="performanceIndicators.length === 0 && !loading" class="col-span-full text-center py-8 text-[#355347]">
            <p class="text-sm">No performance data available</p>
          </div>
          <Card v-for="indicator in performanceIndicators" :key="indicator.metric"
            ...
            <template #content>
              <div class="text-center w-full py-4">
                <div class="text-4xl font-bold text-[#17362b] mb-2">{{ formatNumber(indicator.value) }}</div>
                <div class="text-[1.0rem] font-medium text-[#355347] uppercase tracking-tight leading-tight">{{ getIndicatorLabel(indicator.metric) }}</div>
              </div>
            </template>
          </Card>
        </div>
            ...
            <template #content>
              <div class="space-y-4">
                <div class="flex justify-between items-center">
                  ...
                  <span class="font-semibold">{{ formatNumber(repoHealth.total_archived || 0) }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-[#355347]">Recent Archives (6 months)</span>
                  <span class="font-semibold">{{ formatNumber(repoHealth.recent_archives || 0) }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-[#355347]">Avg. Completion Time</span>
                  <span class="font-semibold">{{ Math.round(repoHealth.avg_completion_days || 0) }} days</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-[#355347]">Well-tagged Submissions</span>
                  <span class="font-semibold">{{ formatPercentage(repoHealth.well_tagged_submissions, repoHealth.total_archived) }}</span>
                </div>
                <ProgressBar
                  :value="repoHealth.total_archived ? (repoHealth.well_tagged_submissions / repoHealth.total_archived) * 100 : 0"
                  class="mt-4"
                  :showValue="false"
                />
              </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card class="shadow-sm rounded-2xl border border-[#cfe0d6] bg-white max-h-96">
            ...
            <template #content>
              <div v-if="adviserWorkload.length === 0 && !loading" class="text-center py-8 text-[#355347]">
