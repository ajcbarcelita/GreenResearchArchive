...
const riskChartOptions = ref({
  ...
  scales: {
    y: {
      title: { display: true, text: 'Revision Count', color: '#17362b', font: { size: 10, weight: 'bold' } },
      min: -0.5,
      max: 5.5,
      grid: { color: '#f0f0f0' },
      ticks: { 
    ...
    x: {
      title: { display: true, text: 'Days of Inactivity', color: '#17362b', font: { size: 10, weight: 'bold' } },
      min: -0.8,
      max: 15.8,
      grid: { color: '#f0f0f0' },
      ticks: { 
...
const fetchData = async () => {
  ...
  try {
    ...
    dashboardMetrics.value[3].value = rows.filter((r) => r.latestSubmissionStatus === 'Approved').length
    ...
    const riskPoints = rows.map((r) => {
      const lastAction = r.latestSubmittedAt ? new Date(r.latestSubmittedAt) : new Date(r.groupCreatedAt)
    ...
    <main class="flex-1 px-4 pt-20 pb-8 sm:px-6 lg:pt-24 flex flex-col">
      <div class="max-w-7xl mx-auto w-full space-y-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card class="shadow-sm rounded-xl border border-[#cfe0d6] bg-white h-[300px] flex flex-col overflow-hidden">
            <template #title>
            ...
            <template #content>
              <div class="flex flex-col h-full px-2">
                <div v-if="phaseStats.total > 0" class="mb-4 p-3 bg-[#f8faf9] rounded-xl border border-[#cfe0d6] flex-none text-[8px]">
                  <div class="flex justify-between font-bold text-[#355347] mb-2 tracking-widest uppercase">
                  ...
                  <div class="flex h-1.5 w-full rounded-full overflow-hidden bg-gray-200 shadow-inner">
                    ...
                    <div class="bg-black" :style="{ width: (phaseStats.archived / phaseStats.total * 100) + '%' }"></div>
                  </div>
                </div>

                <div v-if="advisoryGroups.length" class="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  <div v-for="group in advisoryGroups" :key="group.groupId" class="space-y-1">
                    ...
                    <div class="w-full bg-gray-100 rounded-full h-1">
                      <div class="bg-[#17362b] h-1 rounded-full transition-all duration-500" :style="{ width: getMockProgress(group.latestSubmissionStatus) + '%' }"></div>
                    </div>
                  </div>
                </div>
                <div v-else-if="!loading" class="text-center py-8 text-gray-500 text-xs italic">No active advisees.</div>
              </div>
            </template>
          </Card>
          ...
          <Card class="shadow-sm rounded-xl border border-[#cfe0d6] bg-white h-[300px] flex flex-col overflow-hidden">
            ...
            <template #content>
              <div class="flex-1 min-h-0 px-2 pb-2">
                <Chart type="bubble" :data="riskChartData" :options="riskChartOptions" class="h-full w-full" />
              </div>
            </template>
          </Card>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card class="shadow-sm rounded-xl border border-[#cfe0d6] bg-white h-[175px] flex flex-col overflow-hidden">
            <template #title>
              <div class="flex items-center justify-between px-2 pt-1">
            ...
            <template #content>
              ...
              <div v-else-if="!loading" class="text-center py-8 text-gray-400 text-xs italic px-2">No recent activity.</div>
            </template>
          </Card>
          ...
          <Card class="shadow-sm rounded-xl border border-[#cfe0d6] bg-white h-[175px] flex flex-col overflow-hidden">
            ...
            <template #content>
              <div class="flex flex-col h-full px-2">
                <div v-if="upcomingTasks.length" class="space-y-2 pb-1 overflow-y-auto pr-2 custom-scrollbar">
                  <div v-for="task in upcomingTasks" :key="task.taskId" class="p-2 bg-[#f8faf9] rounded-lg border-l-4 border-[#17362b] border-y border-r border-[#cfe0d6] flex justify-between items-center transition-transform hover:translate-x-1">
                    <div class="flex flex-col min-w-0">
                      <span class="font-bold text-[#17362b] text-[10px] truncate">{{ task.taskName }}</span>
                      <span class="text-[8px] text-[#5a877a]">{{ task.academicYear }}</span>
                    </div>
                    <span class="text-[10px] font-black text-[#355347] whitespace-nowrap uppercase tracking-tighter bg-white px-2 py-1 rounded border border-[#cfe0d6]">{{ formatDeadline(task.dueDate) }}</span>
                  </div>
                </div>
                <div v-else class="text-center py-4 text-gray-400 text-xs italic">No upcoming deadlines.</div>
              </div>
            </template>
          </Card>
        </div>
