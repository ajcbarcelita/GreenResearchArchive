import { listSubmissions, findSubmissionById } from '../models/submissionModel.js'
...
const mapRowToResponse = (row) => {
  return {
    ...
    adviserName: row.adviser_fname && row.adviser_lname ? `Prof. ${row.adviser_fname} ${row.adviser_lname}` : null,
...
export const getRepositoryById = async (req, res) => {
  try {
    ...
    const authors = (membersRes.rows || []).map((m) => `${m.fname} ${m.mname ? m.mname + ' ' : ''}${m.lname}`)
